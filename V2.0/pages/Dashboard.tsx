...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const recentProjects = useMemo(() => {
    // Rank by most recent activity timestamp; fall back to project createdAt
    // so brand-new empty projects still show up at the top.
    const rank = (p: Project): number => {
      const txDates = [
        ...p.expenses.map((e) => (e as any).created_at || e.date),
        ...(p.incomes || []).map((i) => (i as any).created_at || i.date),
      ].filter(Boolean);
      const latestTx = txDates.length
        ? Math.max(...txDates.map((d) => new Date(d).getTime()))
        : 0;
      const created = p.createdAt ? new Date(p.createdAt).getTime() : 0;
      return Math.max(latestTx, created);
    };
    return [...projects]
  ...
  // Budget is stored in each project's own currency — normalize to ILS for sums/ratios.
  const budgetILS = useCallback(
    (p: Project) => convertAmount(p.budget, p.budgetCurrency || 'ILS', 'ILS'),
    [convertAmount]
  );
  ...
  const categoryTotals = useMemo(() => {
    ...
    return categories.map((cat) => {
      ...
      const totalBudget = catProjects.reduce((sum, p) => sum + budgetILS(p), 0);
  ...
  // Normalize each project's budget to ILS before passing to analytics utilities,
  // which compare against spent (always ILS) and assume a single currency.
  const projectsBudgetILS = useMemo(
    () => projects.map((p) => ({ ...p, budget: budgetILS(p) })),
    [projects, budgetILS]
  );
  const overBudgetProjects = useMemo(() => getOverBudgetProjects(projectsBudgetILS), [projectsBudgetILS]);
  const overallBudgetUsage = useMemo(() => getOverallBudgetUsage(projectsBudgetILS), [projectsBudgetILS]);
          ...
          <View style={styles.section}>
            ...
            >
              {recentProjects.map((project) => {
                ...
                const pBudgetILS = budgetILS(project);
                const percentUsed =
                  pBudgetILS > 0
                    ? Math.round((project.spent / pBudgetILS) * 100)
                    : 0;
