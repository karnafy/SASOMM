...
const Projects: React.FC<ProjectsProps> = ({
  ...
  // Budget is stored in each project's own currency — normalize to ILS for sums.
  const budgetILS = (p: Project) =>
    convertAmount(p.budget, p.budgetCurrency || 'ILS', 'ILS');
  // Format budget through its own currency anchor.
  const formatBudget = (p: Project): string =>
    convertAmount(p.budget, p.budgetCurrency || 'ILS', globalCurrency).toLocaleString(undefined, {
      maximumFractionDigits: 0,
    });

  const totals = useMemo(() => {
    const totalBudget = projects.reduce((sum, p) => sum + budgetILS(p), 0);
