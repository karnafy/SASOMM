...
interface AddProjectProps {
  ...
  goBack: () => void;
  onSave: (name: string, budget: number, currency: Currency, category: string, mainCategory?: MainCategory) => Promise<void>;
  project?: Project;
  projects?: Project[];
...
const AddProject: React.FC<AddProjectProps> = ({
  ...
  // Budget is stored in its original currency — no conversion on read/write.
  const [budget, setBudget] = useState(
    project ? project.budget.toFixed(0) : ''
  );
  const [selectedCurrency, setSelectedCurrency] = useState<Currency>(
    project?.budgetCurrency || globalCurrency
  );
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      // Budget is stored AS-IS in the selected currency (no rate conversion here).
      // This prevents drift if exchange rates change between save and display.
      await onSave(
  ...
  }, [isSaving, isAddingCategory, newCategory, category, budget, selectedCurrency, name, mainCategory, onSave, t]);
          ...
          {project && (
            <View style={styles.previousBudgetRow}>
              ...
              <Text style={styles.previousBudgetValue}>
                {CURRENCIES.find((c) => c.val === (project!.budgetCurrency || 'ILS'))?.symbol}
