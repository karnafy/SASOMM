...
interface ProjectDetailProps {
  ...
  project: Project;
  suppliers: Supplier[];
  onUpdateBudget: (projectId: string, newBudget: number, currency: Currency) => Promise<void>;
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({
  ...
  // Budget is stored in project.budgetCurrency — convert only for display, not for editing.
  const budgetCurrency: Currency = project.budgetCurrency || 'ILS';
  const [tempBudget, setTempBudget] = useState(
    convertAmount(project.budget, budgetCurrency, globalCurrency).toFixed(0)
  );
  ...
  // Both sides must be in the same currency for the ratio to be meaningful.
  // remaining is in ILS (sum of expense/income amounts stored as ILS); convert
  // budget to ILS via the project's original currency.
  const budgetInILS = convertAmount(project.budget, project.budgetCurrency || 'ILS', 'ILS');
  const percentUsed = budgetInILS > 0
    ? Math.round((remaining / budgetInILS) * 100)
    : 0;

  useEffect(() => {
    setTempBudget(convertAmount(project.budget, budgetCurrency, globalCurrency).toFixed(0));
  }, [project.budget, budgetCurrency, globalCurrency, convertAmount]);
  ...
  const handleSaveBudget = () => {
    if (onUpdateBudget) {
      // tempBudget is in the currency currently displayed (globalCurrency).
      // Re-anchor the project's budgetCurrency to that as well, so future edits
      // round-trip without rate conversion drift.
      const newBudget = parseFloat(tempBudget) || 0;
      onUpdateBudget(project.id, newBudget, globalCurrency);
    }
          ...
          <div class="report">
            ...
            <div class="summary">
              <div class="summary-card budget">
                ...
                <div class="value">${currencySymbols[globalCurrency]}${convertAmount(project.budget, budgetCurrency, globalCurrency).toLocaleString()}</div>
              </div>
  ...
  // Budget is stored in project.budgetCurrency, not ILS — needs its own formatter.
  const formatBudget = (amount: number): string => {
    return convertAmount(amount, budgetCurrency, globalCurrency).toLocaleString(undefined, {
      maximumFractionDigits: 0,
    });
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.darkZone}>
          ...
          {/* Action Buttons — full-width income/expense buttons matching Dashboard */}
          <View style={styles.bigActionRow}>
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[styles.bigActionLabel, styles.bigActionLabelExpense]}>+ {t('project_detail.add_expense')}</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[styles.bigActionLabel, styles.bigActionLabelIncome]}>+ {t('project_detail.add_income')}</Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  // Action Buttons — match Dashboard's big colored buttons
  bigActionRow: {
